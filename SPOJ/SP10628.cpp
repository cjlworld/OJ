...
const int N=5e5+5,INF=0x3f3f3f3f,SQ=2000;

struct Block
{
	int num[SQ],L[SQ],R[SQ];
	int t[N],F[N];
	int size,k;
	...
	inline int count(rint l,rint r)
	{
		rint i,res=0;
		if(F[l]==F[r]) {
			for(i=l;i<=r;i++) 
		...
		for(i=l;i<=R[F[l]];i++) res+=t[i];
		for(i=F[l]+1;i<=F[r]-1;i++) res+=num[i];
		for(i=L[F[r]];i<=r;i++) res+=t[i];
		return res;
	}
...
int one[N],idx;
int Next[N],ver[N];
inline void AddEdge(rint a,rint b)
{
...
struct Query
{
	...
	inline bool operator<(const Query& t) const
	{
		...
		else return ((l/siz)&1)^(r<t.r);
	}
	Query(int l=0,int r=0,int kth=0,int acs=0,int id=0) :
		l(l),r(r),kth(kth),acs(acs),id(id) {}
}q[N];
...
inline void Add(rint pos)
{
	if(st[pos]) B.Update(a[pos],-1);
	else B.Update(a[pos],1);
	st[pos]^=1;
}
...
//	freopen("1.in","r",stdin);
	...
	for(i=1;i<=m;i++) {
		...
		z=lca(x,y);
		if(z==x) q[i]=Query(lb[x],lb[y],k,0,i);
		else q[i]=Query(rb[x],lb[y],k,z,i);
	}
	...
	for(i=1;i<=m;i++) {
		...
		if(q[i].acs) Add(q[i].acs);
		ans[q[i].id]=B.Getkth(q[i].kth);
		if(q[i].acs) Add(q[i].acs);
	}
	
	for(i=1;i<=m;i++) 
		print(nums[ans[i]-1]),enter;
	return 0;
}
